...
const breadthFirstSearch =(root) => {
    ...
    while(queue.length > 0) // this will check if the length of the line is grater than zero 
    {
        ...
        if(node.left !== null) values.push(node.left); // if data is present in the left side of the node, it will be pushed in to the values array
        if(node.right !== null) values.push(node.right); // if data is present in the right side of the node, it will be pushed in to the values array
